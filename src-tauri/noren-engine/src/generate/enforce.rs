...
pub fn compose_system_prompt(
    ...
    // Extract system prompt template between ```\n and \n```
    let re = Regex::new(r"### System Prompt\s*\n\s*```\n([\s\S]*?)\n```").unwrap();
    let template_match = re.captures(enforcement_prompt).ok_or_else(|| {
        EngineError::PromptNotFound(
            "Could not find system prompt template in enforcement prompt".to_string(),
        )
    })?;
